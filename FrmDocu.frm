...
If frmdocu.LBLEXTORNO.Visible Then
  MsgBox "Impresi�n No Procede...", 48, Pub_Titulo
  Exit Sub
End If
If LOC_TIPMOV = 30 Then
  GoSub ORDEN
...
If LOC_TIPMOV = 10 Then
 If chetrans.Value = 1 And Val(Right(TRANS.Text, 3)) = 0 Then
      MsgBox "Est� Activada a opci�n de Transportista. Seleccione un Transportista ?", 48, Pub_Titulo
     Exit Sub
 End If
End If
Dim I, j
...
If LOC_TIPMOV <> 20 Then Exit Sub
 ...
 cap_valor = InputBox("Modificaci�n de Descto de Mercaderia  en valor en Nuevos Soles S/.= " & Chr(13) & "el valor afecta a costo promedio mas no al documento.", " Descto(%)", d_descto.Caption)
...
If SUMA_VALOR <> 0 Then
 ...
 Do Until fila = grid_fac2.Rows
     ...
     fila = fila + 1
 Loop
pub_mensaje = "Chequear los datos del calculo. Total de Descto en " & d_moneda.Caption & " = " & Format(ww_desc, "0.00") & " - Confirmar la modificaci�n ? "
Pub_Respuesta = MsgBox(pub_mensaje, Pub_Estilo, Pub_Titulo)
If Pub_Respuesta = vbNo Then
   txtnumfac_KeyPress 13
   Exit Sub
...
If LOC_TIPMOV <> 20 Then Exit Sub
 ...
 cap_valor = InputBox("Modificaci�n de Flete en Mercaderia  S/. = ", "Fletes en S/.", d_flete.Caption)
 If cap_valor = "" Then Exit Sub
 If Val(cap_valor) = 0 Then
...
pub_mensaje = "Chequear los datos del calculo. Total de Flete : " & Format(ww_desc, "0.00") & " - Confirmar la modificaci�n ? "
Pub_Respuesta = MsgBox(pub_mensaje, Pub_Estilo, Pub_Titulo)
If Pub_Respuesta = vbNo Then
   txtnumfac_KeyPress 13
   Exit Sub
...
If loc_acceso_descto <> "A" Then
  MsgBox "NO tiene Acceso a esta Opci�n...", 48, Pub_Titulo
  Exit Sub
End If
...
If LOC_TIPMOV = 102 Or OC_TIPMOV = 100 Or LOC_TIPMOV = 10 Or LOC_TIPMOV = 97 Or LOC_TIPMOV = 98 Then
 ...
 If LK_EMP <> "HER" Then
   txtdocu.Caption = "G/ " & far_r!far_serguia & " - " & far_r!far_numguia & " - N�.O/C :" & Trim(Nulo_Valors(far_r!FAR_OC))
 Else
...
If LK_FLAG_GRIFO = "A" And far_r!FAR_tipmov = 20 Then
 d_nomven.Caption = "Turno: " & Format(far_r!far_turno, "00") & Chr(13) & "N�.Carga: " & far_r!FAR_PEDFAC
End If
...
'     fila = fila + 1
   ...
   If LOC_TIPMOV = 99 And Val(far_r!far_codart) = 0 Then
     ...
     ''If all_llave!ALL_SIGNO_CCM <> 0 Then
      grid_fac2.Rows = grid_fac2.Rows + 1
      grid_fac2.TextMatrix(grid_fac2.Rows - 1, 0) = "Relaci�n Conable: "
      grid_fac2.Rows = grid_fac2.Rows + 1
      ...
      End If
      grid_fac2.Rows = grid_fac2.Rows + 1
      grid_fac2.TextMatrix(grid_fac2.Rows - 1, 0) = "' Fec. Cancelaci�n: " & all_llave!ALL_FECHA_CAN
...
If LOC_TIPMOV = 96 Then
   ...
   grid_fac2.TextMatrix(0, 11) = "N�. Cheque"
...
Else
   grid_fac2.Clear
   grid_fac2.Cols = 13
   grid_fac2.TextMatrix(0, 0) = "Descripci�n"
   grid_fac2.TextMatrix(0, 1) = "Codigo"
...
'End If
    ...
    End If
    If LOC_TIPMOV = 20 Then
        frmdocu.Reportes.WindowTitle = "KARDEX N� :" & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "0000000")
