...
Begin VB.Form frmPesos 
   Caption         =   "Transfer. Import. (Recepci�n de Informaci�n)"
   ClientHeight    =   6900
   ClientLeft      =   555
   ClientTop       =   1845
   ClientWidth     =   5310
   ...
   Begin VB.Frame fracabe 
      ...
      End
      Begin VB.OptionButton openvio 
         Caption         =   "Transfer Recepci�n de Mercaderia"
         ForeColor       =   &H00808000&
         Height          =   195
         Index           =   0
         Left            =   840
   ...
   Begin VB.Label lblpro 
      Caption         =   "Procesando Informaci�n ..."
      ForeColor       =   &H00808000&
      Height          =   255
      Left            =   240
      TabIndex        =   13
   ...
   Begin VB.Label Label6 
      Caption         =   "Descripci�n de Articulo en Base Transfer :"
      ForeColor       =   &H00808000&
      Height          =   255
      Left            =   240
      TabIndex        =   11
   ...
   Begin VB.Label Label5 
      Caption         =   "Descripci�n de Articulo en Base del Servidor :"
      ForeColor       =   &H00808000&
      Height          =   255
      Left            =   240
      TabIndex        =   10
...
MsgBox "Pase de la Informaci�n del Transfer. ha sido Anulado ", 48, Pub_Titulo
...
If Trim(Nulo_Valors(GEN!gen_ciaimp)) = "" Then
  MsgBox "El codigo de la Compa�ia de la Recepcion no Validad... Cia : " & Trim(Nulo_Valors(GEN!gen_ciaimp)), 48, Pub_Titulo
  Exit Sub
End If
If LK_CODCIA <> Trim(Nulo_Valors(GEN!gen_ciaimp)) Then
   MsgBox "El Codigo de esta Compa�ia no es validad para el proceso... " & Chr(13) & "Compa�ia validad : " & Trim(Nulo_Valors(GEN!gen_ciaimp)), 48, Pub_Titulo
   GoTo fin
End If

VENTAS.DatabaseName = lblarchi.Caption '  Left(Trim(PUB_RUTA_OTRO), 1) & ":\ADMIN\Standar\TAjustes.mdb"
PUB_IMPORTE = 0
WCONFIG = InputBox("- Digitar 1 para fechas con formato ##/##/#### " & Chr(13) & "- Digitar 2 para fechas con formato ##/##/## ", "Cofiguraci�n del TRANSFER", 2)
If WCONFIG = "" Then
  Exit Sub
End If
...
If VENTAS.Recordset.EOF Then
  Screen.MousePointer = 0
  MsgBox "VERIFICAR ...NO HAY DATOS EN EL ARCHIVO : " & Trim(lblarchi.Caption) & Chr(13) & "Posible Soluciones :" & Chr(13) & "El formato de fecha no es correcta  0  la fecha del dia de la compa�ia no es la misma del archivo transfer. O el archivo esta vacio o da�ado.", 48, Pub_Titulo
  VENTAS.RecordSource = "SELECT * FROM FACART "
  VENTAS.Refresh
  If Val(WCONFIG) = 1 Then
    If Not VENTAS.Recordset.EOF Then MsgBox "Datos el Transfer son del d�a : " & VENTAS.Recordset!FAR_fecha & Chr(13) & "y la fecha de la compa�ia es del dia : " & Format(LK_FECHA_DIA, "dd/mm/yyyy"), vbInformation, Pub_Titulo
  Else
    If Not VENTAS.Recordset.EOF Then MsgBox "Datos el Transfer son del d�a : " & VENTAS.Recordset!FAR_fecha & Chr(13) & "y la fecha de la compa�ia es del dia : " & Format(LK_FECHA_DIA, "dd/mm/yy"), vbInformation, Pub_Titulo
  End If
  Exit Sub
End If
GoSub WEXCEL
...
If AVISO = 1 Then
   MsgBox "Ya proceso los datos del d�a ", 48, Pub_Titulo
   GoTo fin
End If
