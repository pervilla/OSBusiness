...
Begin VB.Form frmPlanS 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tabs            =   2
      TabHeight       =   520
      TabCaption(0)   =   "Stock Minimoas y Maximos"
      TabPicture(0)   =   "frmPresP.frx":014A
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "fraListar"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Definicion de Escalas de Productos"
      TabPicture(1)   =   "frmPresP.frx":0166
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "Label2"
      Tab(1).Control(1)=   "gridesca"
      Tab(1).Control(2)=   "tvar"
      Tab(1).Control(3)=   "cmdlineas"
      Tab(1).Control(4)=   "optipo(0)"
      Tab(1).Control(5)=   "optipo(1)"
      Tab(1).Control(6)=   "cmdmostrar"
      Tab(1).Control(7)=   "pbesca"
      Tab(1).ControlCount=   8
      Begin MSComctlLib.ProgressBar pbesca 
         Height          =   225
         Left            =   -74400
         TabIndex        =   25
         Top             =   1200
      ...
      Begin VB.ComboBox cmdlineas 
         ...
         Width           =   3615
      End
      Begin RichTextLib.RichTextBox tvar 
         Height          =   375
         Left            =   -64920
         TabIndex        =   18
         Top             =   1680
      ...
      Begin VB.Frame fraListar 
         ...
         Begin VB.CheckBox cheprecios 
            ...
            Width           =   2535
         End
         Begin VB.CheckBox cheprecios2 
            Caption         =   "Lista de Precios Opciones 2 "
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   12
               Charset         =   0
         ...
         End
      End
      Begin MSFlexGridLib.MSFlexGrid gridesca 
         Height          =   5805
         Left            =   -74760
         TabIndex        =   21
         Tag             =   "9999"
...
Private Sub cheprecios2_Click()
If cheprecios2.Value = 1 Then
 lisarti.Clear
 Screen.MousePointer = 11
 LLENA_ARTI -99, 0
 Screen.MousePointer = 0
...
Private Sub cmdlineas_KeyPress(KeyAscii As Integer)
If KeyAscii = 13 Then
 cmdmostrar.SetFocus
End If
End Sub
...
LLENA_ARTI_ESCALAS Val(Right(cmdlineas.Text, 10))
End Sub
...
Dim CONTA As Integer
    ...
    Loop
End Sub
Public Sub LLENADOScombo(cont As ComboBox, tip As Integer)
Dim CONTA As Integer
    CONTA = -1
    PUB_TIPREG = tip
    SQ_OPER = 2
...
If gridarti.COL <> 16 And gridarti.COL <> 6 And gridarti.COL <> 9 And gridarti.COL <> 10 Then Exit Sub
'veri_suma
...
Private Sub gridesca_KeyPress(KeyAscii As Integer)
Dim a As Integer
Dim t, WC
Static CONS
If KeyAscii <> 13 Then Exit Sub

If gridesca.COL <> 16 And gridesca.COL <> 6 And gridesca.COL <> 9 And gridesca.COL <> 10 Then Exit Sub
'veri_suma
    tvar.Left = gridarti.Left + gridarti.CellLeft
    tvar.Width = gridarti.CellWidth
    tvar.Height = gridarti.CellHeight
    tvar.Top = gridarti.Top + gridarti.CellTop
    tvar.Text = gridarti.TextMatrix(gridarti.Row, gridarti.COL)
    tvar.Visible = True
    Azul3 tvar, tvar
    tvar.SetFocus
'End If
...
If chepedmax.Value = 1 Then
    pub_cadena = "update articulo set arm_stock2 = " & Val(gridarti.TextMatrix(gridarti.Row, 9)) * Val(gridarti.TextMatrix(gridarti.Row, 15)) & " where arm_codart in (" & PUB_KEY & ") and arm_codcia in('25','27','28','29','09') "
    CN.Execute pub_cadena
    GoTo mstmaxp
End If
If cheprecios2.Value = 1 Then
   'pub_cadena = "SELECT PRE_OP1,PRE_CANT FROM PRECIOS WHERE PRE_CODART = " & PUB_KEY & " AND PRE_EQUIV = " & Val(gridarti.TextMatrix(gridarti.Row, 15))
   ...
   '    X!PRE_CANT = Val(Format(Val(gridarti.TextMatrix(gridarti.Row, 16)) * Val(gridarti.TextMatrix(gridarti.Row, 15)), "0.00"))
   '    X.Update
   '    X.MoveNext
   'Loop
   pub_cadena = "UPDATE PRECIOS SET PRE_OP2 = " & Val(gridarti.TextMatrix(gridarti.Row, 6)) & " ,PRE_CANT = " & Val(Format(Val(gridarti.TextMatrix(gridarti.Row, 16)) * Val(gridarti.TextMatrix(gridarti.Row, 15)), "0.00")) & " FROM PRECIOS WHERE PRE_CODART = " & PUB_KEY & " AND PRE_EQUIV = " & Val(gridarti.TextMatrix(gridarti.Row, 15))
   CN.Execute pub_cadena
...
If cheprecios.Value = 1 Then
   ...
   '    X.MoveNext
   'Loop
   pub_cadena = "UPDATE PRECIOS SET PRE_OP1 = " & Val(gridarti.TextMatrix(gridarti.Row, 6)) & " ,PRE_CANT = " & Val(Format(Val(gridarti.TextMatrix(gridarti.Row, 16)) * Val(gridarti.TextMatrix(gridarti.Row, 15)), "0.00")) & " FROM PRECIOS WHERE PRE_CODART = " & PUB_KEY & " AND PRE_EQUIV = " & Val(gridarti.TextMatrix(gridarti.Row, 15))
   CN.Execute pub_cadena
...
If cheprecios.Value = 1 Then
  pub_cadena = "SELECT ARM_COSPRO, ART_STOCK_MAX , PRE_COSTO_ANT ,PRE_CANT, ART_KEY , art_alterno, art_nombre, PRE_EQUIV , PRE_UNIDAD,pre_op1,pre_pre2, art_stockreal_min, art_stockreal_max, ARM_STOCK  , ((ARM_STOCK / PRE_EQUIV) - (ART_STOCK_MAX / PRE_EQUIV)) AS DIF , ART_PLANCHA FROM ARTI, articulo, precios WHERE (ART_CODCIA = ARM_CODCIA) AND (ART_KEY = ARM_CODART) AND (ART_CODCIA = PRE_CODCIA) AND (ART_KEY = PRE_CODART ) AND PRE_FLAG_UNIDAD = 'A' AND ART_CODCIA = ? AND ART_ORDEN = 1 AND ART_SITUACION <> '1' AND ART_CALIDAD = 1 and ART_FLAG_STOCK = 'M' ORDER BY art_nombre " 'DIF ASC   "
  wvalor = 0
ElseIf cheprecios2.Value = 1 Then
  pub_cadena = "SELECT ARM_COSPRO, ART_STOCK_MAX , PRE_COSTO_ANT ,PRE_CANT, ART_KEY , art_alterno, art_nombre, PRE_EQUIV , PRE_UNIDAD,pre_op2,pre_pre2, art_stockreal_min, art_stockreal_max, ARM_STOCK  , ((ARM_STOCK / PRE_EQUIV) - (ART_STOCK_MAX / PRE_EQUIV)) AS DIF , ART_PLANCHA FROM ARTI, articulo, precios WHERE (ART_CODCIA = ARM_CODCIA) AND (ART_KEY = ARM_CODART) AND (ART_CODCIA = PRE_CODCIA) AND (ART_KEY = PRE_CODART ) AND PRE_FLAG_UNIDAD = 'A' AND ART_CODCIA = ? AND ART_POR6 = 1 AND ART_SITUACION <> '1' AND ART_CALIDAD = 1 and ART_FLAG_STOCK = 'M' ORDER BY art_nombre " 'DIF ASC   "
  wvalor = 0
Else
   If wfami = 0 Then Exit Sub
  'If wfami <> 0 Then
   pub_cadena = "SELECT ARM_COSPRO, ART_STOCK_MAX , PRE_COSTO_ANT , PRE_CANT, ART_KEY , art_alterno, art_nombre, PRE_EQUIV , PRE_UNIDAD, art_stockreal_min, art_stockreal_max, ARM_STOCK  , ((ARM_STOCK / PRE_EQUIV) - (ART_STOCK_MAX / PRE_EQUIV)) AS DIF , ART_PLANCHA FROM ARTI, articulo, precios WHERE (ART_CODCIA = ARM_CODCIA) AND (ART_KEY = ARM_CODART) AND (ART_CODCIA = PRE_CODCIA) AND (ART_KEY = PRE_CODART ) AND PRE_FLAG_UNIDAD = 'A' AND ART_CODCIA = ? AND ART_FAMILIA = ? AND ART_CALIDAD = 1 and ART_FLAG_STOCK = 'M' " & CADE_ROT & " ORDER BY art_nombre " 'DIF ASC   "
   wvalor = wfami
End If
...
If cheprecios.Value = 1 Or cheprecios2.Value = 1 Then
...
gridarti.ColWidth(16) = 0
If cheprecios.Value = 1 Or cheprecios2.Value = 1 Then
    gridarti.TextMatrix(0, 16) = "Cantid."
    gridarti.TextMatrix(1, 16) = "Minima"
...
'  If Val(wllave_arti!PRE_COSTO_ANT) = 1610.7 Then Stop
  ...
  pub_cadena = "SELECT SUM(ARM_STOCK) AS STOCK FROM ARTICULO WHERE ARM_CODCIA IN ('25','27','28','29','10') AND ARM_CODART = " & wllave_arti!art_key
  Set RSSTOCKOT = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues)   ' rdConcurReadOnly) ', rdConcurLock)
   gridarti.TextMatrix(xfil, 5) = Format(Val(RSSTOCKOT!stock) / Val(wllave_arti!PRE_EQUIV), "0.00")
  If cheprecios.Value = 1 Or cheprecios2.Value = 1 Then
    If cheprecios2.Value = 1 Then
      gridarti.TextMatrix(xfil, 6) = Format(Nulo_Valor0(wllave_arti!PRE_OP2), "0.00")
    Else
      gridarti.TextMatrix(xfil, 6) = Format(Nulo_Valor0(wllave_arti!PRE_OP1), "0.00")
    End If
    gridarti.TextMatrix(xfil, 5) = Format((0 + Val(RSSTOCKOT!stock)) / Val(wllave_arti!PRE_EQUIV), "0.00")
    gridarti.TextMatrix(xfil, 8) = Trim(wllave_arti!PRE_PRE2)
    gridarti.TextMatrix(xfil, 16) = Format(Val(Nulo_Valor0(wllave_arti!PRE_CANT)) / Val(wllave_arti!PRE_EQUIV), "0.00")
    GoTo mstmaxp
  End If
  
  
  pub_cadena = "SELECT SUM(ART_STOCK_MAX) AS STOCK FROM ARTI WHERE ART_CODCIA IN ('25','27','28','29','10') AND ART_KEY = " & wllave_arti!art_key
  Set RSSTOCKOT = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues)   ' rdConcurReadOnly) ', rdConcurLock)
  If Not RSSTOCKOT.EOF Then gridarti.TextMatrix(xfil, 6) = Format(RSSTOCKOT!stock / wllave_arti!PRE_EQUIV, "0.00")
...
End Sub
Public Sub LLENA_ARTI_ESCALAS(wfami As Integer)
Dim RSSTOCKOT As rdoResultset
If pbesca.Visible = True Then Exit Sub
Dim xpinta As Integer
Dim xfil  As Integer
Dim WARTI As rdoQuery
Dim wllave_arti As rdoResultset
...
pub_cadena = "SELECT ARM_COSPRO, ART_STOCK_MAX , PRE_COSTO_ANT ,PRE_CANT, ART_KEY , art_alterno, art_nombre, PRE_EQUIV , PRE_UNIDAD,pre_op1,pre_pre2, art_stockreal_min, art_stockreal_max, ARM_STOCK  , ((ARM_STOCK / PRE_EQUIV) - (ART_STOCK_MAX / PRE_EQUIV)) AS DIF , ART_PLANCHA FROM ARTI, articulo, precios WHERE (ART_CODCIA = ARM_CODCIA) AND (ART_KEY = ARM_CODART) AND (ART_CODCIA = PRE_CODCIA) AND (ART_KEY = PRE_CODART ) AND PRE_FLAG_UNIDAD = 'A' AND ART_CODCIA = ? AND ART_SITUACION <> '1' AND ART_CALIDAD = 1 and ART_FLAG_STOCK = 'M' AND ART_FAMILIA = " & wfami & " ORDER BY art_nombre " 'DIF ASC   "
Set WARTI = CN.CreateQuery("", pub_cadena)
WARTI(0) = " "
If wvalor <> 0 Then WARTI(1) = 0
Set wllave_arti = WARTI.OpenResultset(rdOpenKeyset, rdConcurValues)
...
pbesca.Value = 0
pbesca.Min = 0
pbesca.max = wllave_arti.RowCount
pbesca.Visible = True
gridesca.Visible = False
DoEvents
gridesca.Clear
gridesca.Cols = 20
gridesca.TextMatrix(0, 0) = "Item"
gridesca.TextMatrix(0, 1) = "Descripcion del Producto"
gridesca.TextMatrix(0, 2) = "Unid."
gridesca.TextMatrix(0, 2) = "Activa"
gridesca.TextMatrix(0, 3) = "Costo"
gridesca.TextMatrix(1, 3) = "Repos."
gridesca.TextMatrix(0, 4) = "Costo"
gridesca.TextMatrix(1, 4) = "Ptromd"
gridesca.TextMatrix(0, 5) = "Stock "
gridesca.TextMatrix(1, 5) = "Actual "
gridesca.TextMatrix(0, 6) = "Stock "
gridesca.TextMatrix(1, 6) = "Maximo"
gridesca.ColWidth(7) = 0
gridesca.ColWidth(8) = 0
gridesca.ColWidth(9) = 0

gridesca.TextMatrix(0, 10) = "Esc-1-" & verFlag
gridesca.TextMatrix(1, 10) = "Inicial"

gridesca.TextMatrix(0, 11) = "Esc-1-" & verFlag
gridesca.TextMatrix(1, 11) = "Final"

gridesca.TextMatrix(0, 12) = "Esc-1-" & verFlag
gridesca.TextMatrix(1, 12) = "Valor"

gridesca.TextMatrix(0, 13) = "Esc-2-" & verFlag
gridesca.TextMatrix(1, 13) = "Inicial"

gridesca.TextMatrix(0, 14) = "Esc-2-" & verFlag
gridesca.TextMatrix(1, 14) = "Final"

gridesca.TextMatrix(0, 15) = "Esc-2-" & verFlag
gridesca.TextMatrix(1, 15) = "Valor"

gridesca.TextMatrix(0, 16) = "Esc-3-" & verFlag
gridesca.TextMatrix(1, 16) = "Inicial"

gridesca.TextMatrix(0, 17) = "Esc-3-" & verFlag
gridesca.TextMatrix(1, 17) = "Final"

gridesca.TextMatrix(0, 18) = "Esc-3-" & verFlag
gridesca.TextMatrix(1, 18) = "Valor"

gridesca.ColWidth(19) = 0

gridesca.Rows = 2
xfil = 1
Do Until wllave_arti.EOF
  pbesca.Value = pbesca.Value + 1
  DoEvents
  xfil = xfil + 1
  gridesca.Rows = gridesca.Rows + 1
  gridesca.RowHeight(gridesca.Rows - 1) = 285
  lisarti.AddItem wllave_arti!ART_alterno & " " & wllave_arti!art_nombre & String(120, " ") & wllave_arti!art_key
  gridesca.TextMatrix(xfil, 0) = Format(xfil - 1, "00")
  gridesca.TextMatrix(xfil, 1) = Trim(wllave_arti!art_nombre)
  gridesca.TextMatrix(xfil, 2) = Trim(wllave_arti!PRE_UNIDAD)
  gridesca.TextMatrix(xfil, 3) = Format(wllave_arti!PRE_cosTO_ant, "0.00")
  gridesca.TextMatrix(xfil, 4) = Format(wllave_arti!ARM_COSPRO * wllave_arti!PRE_EQUIV, "0.00")
    ...
    Set RSSTOCKOT = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues)   ' rdConcurReadOnly) ', rdConcurLock)
    If cheprecios2.Value = 1 Then
      gridesca.TextMatrix(xfil, 6) = Format(Nulo_Valor0(wllave_arti!PRE_OP2), "0.00")
    Else
      gridesca.TextMatrix(xfil, 6) = Format(Nulo_Valor0(wllave_arti!PRE_OP1), "0.00")
    End If
sale_otro:
wllave_arti.MoveNext
Loop
'sum_costo_linea
gridesca.Visible = True
gridesca.SetFocus
pbesca.Visible = False
DoEvents
End Sub


Private Sub tvar_KeyPress(KeyAscii As Integer)
If KeyAscii = 27 Then
  tvar.Text = temporal
  tvar.Visible = False
  gridesca.SetFocus
