...
Public lot_llave2 As rdoResultset
Public PSLOT_LLAVE2 As rdoQuery
...
Public pub_numoper As Long
Public pu_codtra As Integer
Public pu_codCaja As Integer
...
Public PSCAR_TOPE As rdoQuery
Public car_tope  As rdoResultset

Public flag_lotes_chequea As String * 1
Public flag_autoriza_pase As String * 1
...
pub_cadena = "SELECT * FROM CLIENTES WHERE CLI_CP= '" & pu_cp & "' AND CLI_CODCLIE  = " & pu_codclie & " AND CLI_CODCIA = '" & pu_codcia & "' ORDER BY CLI_CP ,CLI_CODCLIE"
Set cli_llave = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
...
Public Sub CONEXION_GEN()
 ...
 'CONn$ = "dsn=" & wdsn & ";uid=jorge;pwd=jjj99;database=bdatos;"
  ...
  pub_cadena = "SELECT  CLI_CODCLIE, CLI_ESTADO, CLI_NOMBRE FROM CLIENTES WHERE CLI_CP=? AND CLI_RUC_ESPOSO = ? AND CLI_CODCIA = ? "
  ...
  Pro_Aumento 15
  Splash.lblmensa.Caption = "Identicando relación de archivos..."
  DoEvents
  ...
  pub_cadena = "SELECT * FROM SUB_TRANSA WHERE SUT_CODTRA = ?  ORDER BY SUT_ORDEN ,  SUT_DESCRIPCION"
  ...
  pub_cadena = "SELECT * FROM CHEQUES WHERE CHE_NUMOPER= ? AND CHE_FECHA=? AND CHE_CODBAN = ? AND CHE_IMPORTE = ?  ORDER BY CHE_FECHA,CHE_NUMOPER"
  ...
  pub_cadena = "SELECT * FROM CHEQUES WHERE CHE_CODBAN = ? AND CHE_CODCIA=?  AND CHE_FECHA_EMISION >= ? ORDER BY CHE_CODBAN, CHE_CODCIA, CHE_FECHA_EMISION, CHE_SIGNO_CCM DESC, CHE_NUMOPER2"
  ...
  pub_cadena = "SELECT * FROM CARTERA WHERE  CAR_CODCIA = ? AND CAR_CP = ? AND CAR_CODCLIE = ?  AND CAR_IMPORTE <> 0 ORDER BY  CAR_TIPDOC , CAR_FECHA_SUNAT "
...
'  Splash.rctStatusBar.Value = Splash.rctStatusBar.Value + 100
  ...
  pub_cadena = "SELECT * FROM LOTE WHERE LOT_CODCIA = ? AND LOT_CODART = ? AND LOT_NROLOTE = ? ORDER BY LOT_FECHA_VCTO"
  ...
  pub_cadena = "SELECT * FROM LOTE WHERE LOT_CODCIA = ? AND LOT_CODART = ? ORDER BY LOT_FECHA_VCTO"
  Set PSLOT_LLAVE2 = CN.CreateQuery("", pub_cadena)
  PSLOT_LLAVE2(0) = 0
  PSLOT_LLAVE2(1) = 0
  Set lot_llave2 = PSLOT_LLAVE2.OpenResultset(rdOpenKeyset, rdConcurValues)
  ...
  pub_cadena = "SELECT PAR_CODCIA, PAR_NOMBRE, PAR_NOMBRE_CORTO, PAR_DIR_EMP, PAR_COLOR FROM PARGEN WHERE PAR_CODCIA = ?  ORDER BY PAR_CODCIA "
...
'  Set ser_llave = PSSER_LLAVE.OpenResultset(rdOpenKeyset, rdConcurValues)
  ...
  Pro_Aumento 62
  Splash.lblmensa.Caption = "Terminando las concexiones ..."
  DoEvents
...
If LK_EMP = "HER" Then pu_codcia = par_llave!PAR_CIACCM
Select Case SQ_OPER
Case 1
...
pub_cadena = "SELECT CAR_IMP_INI,  CAR_CODCLIE, CAR_CODCIA, CAR_SERDOC, CAR_NUMDOC, CAR_TIPDOC  FROM CARTERA WHERE CAR_CODCIA = '" & pu_codcia & "' AND CAR_NUMOPER = " & PUB_NUM_OPER_EXT & " AND CAR_FECHA_INGR = '" & Format(PUB_FECHA, "dd/mm/yyyy") & "' "
Set car_far2 = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
...
pub_cadena = "SELECT * FROM facart WHERE FAR_FECHA = '" & Format(PUB_FECHA, "dd/mm/yyyy") & "' AND FAR_NUMOPER = " & PUB_NUM_OPER_EXT & " AND FAR_CODCIA = '" & LK_CODCIA & "' AND FAR_ESTADO <> 'E' ORDER BY  FAR_NUMSEC"
...
pub_cadena = "SELECT FAR_CODCLIE, FAR_NUMOPER , FAR_FECHA, FAR_FBG,FAR_NUMSER, FAR_NUMFAC, FAR_NUMFAC_C, FAR_NUMSER_C , FAR_NUMFAC, FAR_NUMSER, FAR_FBG, FAR_BRUTO, FAR_IMPTO, FAR_SUBTOTAL, FAR_FECHA_COMPRA , FAR_NUMDOC, FAR_MONEDA, FAR_CODCIA, FAR_TIPMOV , far_tipdoc, FAR_DIAS FROM facart WHERE FAR_TIPMOV = " & PU_TIPMOV & " AND FAR_CODCIA = '" & pu_codcia & "' AND FAR_NUMSER_C = '" & PU_NUMSER & "' AND (FAR_FBG IN ('" & PU_FBG & "','" & PU_FBG2 & "')) AND FAR_NUMFAC_C = '" & PU_NUMFAC & "' AND far_estado <> 'E'  ORDER BY FAR_TIPMOV, FAR_CODCIA, FAR_NUMSER, FAR_FBG, FAR_NUMFAC, FAR_NUMSEC"
Set far_menor4 = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
...
PSCHE_OPER(2) = PUB_CODBAN
PSCHE_OPER(3) = PUB_IMPORTE
che_oper.Requery
...
Public Function alta_vista_nombre(WLV1 As ListView, TEXTO As String, wcp As String, Optional WBUS) As Integer
...
If Left(TEXTO, 1) = "%" Then
  ...
  archi = "SELECT CLI_CODCLIE, CLI_CODCIA, CLI_CP, CLI_NOMBRE_EMPRESA,CLI_CASA_DIREC,CLI_ZONA_NEW, CLI_CASA_NUM,TAB_NOMLARGO FROM CLIENTES,TABLAS WHERE  CLI_CP = '" & wcp & "' AND CLI_CODCIA = '" & LK_CODCIA & "' AND (TAB_CODCIA = '00') AND (CLI_ZONA_NEW = TAB_NUMTAB) AND TAB_TIPREG = 35 AND CLI_NOMBRE_EMPRESA LIKE '%" & TEXTO & "%' ORDER BY CLI_NOMBRE_EMPRESA"
  GoTo LISTO_SELE
End If

If Not IsMissing(WBUS) Then
  If WBUS = "D" Then
  archi = "SELECT CLI_CODCLIE, CLI_CODCIA, CLI_CP, CLI_NOMBRE,CLI_CASA_DIREC,CLI_ZONA_NEW, CLI_CASA_NUM,TAB_NOMLARGO FROM CLIENTES,TABLAS WHERE  CLI_CP = '" & wcp & "' AND CLI_CODCIA = '" & LK_CODCIA & "' AND (TAB_CODCIA = '00') AND (CLI_ZONA_NEW = TAB_NUMTAB) AND TAB_TIPREG = 35 AND CLI_CASA_DIREC LIKE '%" & TEXTO & "%' ORDER BY CLI_NOMBRE"
  End If
Else
  archi = "SELECT CLI_CODCLIE, CLI_CODCIA, CLI_CP, CLI_NOMBRE,CLI_CASA_DIREC,CLI_ZONA_NEW, CLI_CASA_NUM,TAB_NOMLARGO FROM CLIENTES,TABLAS WHERE  CLI_CP = '" & wcp & "' AND CLI_CODCIA = '" & LK_CODCIA & "' AND (TAB_CODCIA = '00') AND (CLI_ZONA_NEW = TAB_NUMTAB) AND TAB_TIPREG = 35 AND CLI_NOMBRE LIKE '%" & TEXTO & "%' ORDER BY CLI_NOMBRE"
End If
LISTO_SELE:
...
'pub_cadena = "SELECT ALL_NUMOPER FROM ALLOG WHERE ALL_CODCIA = '" & pu_codcia & "' AND ALL_FECHA_DIA = '" & Format(PUB_FECHA, "dd/mm/yyyy") & "'   ORDER BY ALL_NUMOPER DESC "
'Set all_menor = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
