...
Else
   If frmBancos.TXTBAN(2).Text = "" Or Len(frmBancos.TXTBAN(2).Text) = 0 Then
       MsgBox "Ingrese Descripci�n de Banco ..!!!", 48, Pub_Titulo
...
If Left(cmdcontab.Caption, 2) = "&Q" Then
    pub_mensaje = "Confirmar la eliminaci�n de la Cuenta : " & Trim(TXTBAN(4).Text) & " , Continuar ?"
...
If Trim(frmBancos.TXTBAN(4).Text) <> "" Then
    ...
    Else
      ...
      'If Not com_llave.EOF Then
       ...
       '    pub_mensaje = "Desea tambien la Descripci�n de la Cta. Contable. ?"
       '    Pub_Respuesta = MsgBox(pub_mensaje, Pub_Estilo, Pub_Titulo)
