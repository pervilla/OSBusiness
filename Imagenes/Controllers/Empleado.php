...
class Empleado extends BaseController {
    ...
    public function creditos_adelantos() {
        return view('empleado/index_creditos');
    }

    public function rentabilidad() {

        return view('empleado/index_rentabilidad');
    }
    public function get_rentabilidad(){
        ...
        $comisiones = $this->Facart_model->get_rentabilidad_empleado($mes, $anio);
        $this->Facart3_model = new Server03Model();
        $comisiones3 = $this->Facart3_model->get_rentabilidad_empleado($mes, $anio,3);
        $comisiones2 = $this->Facart3_model->get_rentabilidad_empleado($mes, $anio,2);
        $comiT = array();
        foreach ($comisiones as $comision) {
            ...
            $comi['COMISION'] = number_format((float)$comision->VENTA, 2, '.', '');
            $comi['COSTO'] = number_format((float)$comision->COSTO, 2, '.', '');
            $comi['COMISION3'] = 0;
            $comi['COSTO3'] = 0;
            $comi['COMISION2'] = 0;
            $comi['COSTO2'] = 0;
            $comi['TOTAL'] = number_format((float)$comision->VENTA, 2, '.', '');
            $comi['TOTALCOST'] = number_format((float)$comision->COSTO, 2, '.', '');
            $comi['AVANCE'] = $comision->VEM_META > 0 ? number_format((float)(($comision->VENTA) * 100 / $comision->VEM_META), 2, '.', '') : 0;
            foreach ($comisiones3 as $comision3) {
                if ($comision->VEM_CODVEN == $comision3->VEM_CODVEN) {
                    ...
                    $comi['TOTAL'] = number_format((float)$comision->VENTA + $comision3->VENTA, 2, '.', '');
                    $comi['TOTALCOST'] = number_format((float)$comision->COSTO + $comision3->COSTO, 2, '.', '');
                    $comi['AVANCE'] = $comision->VEM_META > 0 ? number_format((float)(($comision->VENTA + $comision3->VENTA) * 100 / $comision->VEM_META), 2, '.', '') : 0;
                }
            }
            foreach ($comisiones2 as $comision2) {
                if ($comision->VEM_CODVEN == $comision2->VEM_CODVEN) {
                    ...
                    $comi['TOTAL'] = number_format((float)$comision->VENTA + $comision2->VENTA, 2, '.', '');
                    $comi['TOTALCOST'] = number_format((float)$comision->COSTO + $comision2->COSTO, 2, '.', '');
                    $comi['AVANCE'] = $comision->VEM_META > 0 ? number_format((float)(($comision->VENTA + $comision2->VENTA) * 100 / $comision->VEM_META), 2, '.', '') : 0;
                }
            }
    ...
    public function get_creditos() {
        ...
        $this->CajaMovimientosModel = new CajaMovimientosModel();
        $creditos = $this->CajaMovimientosModel->get_creditos($mes, $anio);
    ...
    public function get_creditos_empleado() {
        ...
        $this->CajaMovimientosModel = new CajaMovimientosModel();
        $movi = $this->CajaMovimientosModel->get_creditos_empleado($request->getPost('mes'), $request->getPost('anio'), $request->getPost('empl'));
