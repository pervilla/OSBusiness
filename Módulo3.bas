...
Public LK_ACCESO_EXTORNO As String * 1
Public PUB_LOT_CANT As Currency
Public PUB_CODLOT  As String
Public PUB_FECHA_LOT As Date
      
Public LK_FLAG_SERIES As String * 1
Public LK_RUTA_DOC As String
...
Public PUB_NUM_OPER As Currency
Public PUB_NUM_OPER_EXT As Currency
Public PUB_NUM_OPER_XXX As Currency
...
Public pu_alterno As String * 15
...
'VARIABLES DE RELACION CONTABLE ONLYONT
   ...
   Public lk_descrip_cias As String * 20
   Public flag_cambio_pre As String * 1
   Public USU_ACC_TIPDOC As String
   Public LK_CLAVE_GERENCIA As String
   Public LK_FLAG_PARTES As String
   
Public ws_sut_transa As Integer

' API para SendKeys seguro y area de trabajo
Private Declare Function keybd_event Lib "user32" (ByVal bVk As Byte, ByVal bScan As Byte, ByVal dwFlags As Long, ByVal dwExtraInfo As Long) As Long
Private Declare Function GetKeyboardState Lib "user32" (ByVal pbKeyState As Long) As Long
Private Declare Function SetKeyboardState Lib "user32" (ByVal lppbKeyState As Long) As Long
Private Declare Function GetForegroundWindow Lib "user32" () As Long
Private Declare Function SetForegroundWindow Lib "user32" (ByVal hwnd As Long) As Long

' API para obtener area de trabajo de Windows (evitar taskbar)
...
Public Const VK_UP = &H26      ' Up arrow
Public Const VK_DOWN = &H28    ' Down arrow
Public Const VK_HOME = &H24    ' Home key
Public Const VK_NUMLOCK = &H90 ' NumLock key

Private Declare Function GetKeyState Lib "user32" (ByVal nVirtKey As Long) As Integer

Public Sub PreservarNumLock()
    On Error Resume Next
    Dim numLockState As Integer
    numLockState = GetKeyState(VK_NUMLOCK)
    ' Si NumLock esta encendido (bit 1 = 1) pero se apago, reactivarlo
    If (numLockState And 1) = 1 Then
...
Public Sub AlternarVentanaFormulario(frm As Form)
    On Error Resume Next
...
Public Sub CenterMe(frm As Form)
    ...
    ' Get working area to avoid taskbar
    Dim rc As RECT
    If SystemParametersInfo(SPI_GETWORKAREA, 0, rc, 0) Then
        screenWidth = (rc.Right - rc.Left)
        screenHeight = (rc.Bottom - rc.Top)
    End If
    
    frm.Left = (screenWidth * Screen.TwipsPerPixelX - frm.Width) \ 2
    frm.Top = (screenHeight * Screen.TwipsPerPixelY - frm.Height) \ 2
End Sub
...
'   MsgBox FORMGEN.Controls(i).Name
        ...
        tab_mayor!TAB_NOMLARGO = FORMGEN.Controls(I).Name
        tab_mayor.Update
        C = C + 1
      End If
 Next I
...
'Set X = CN.OpenResultset(archi, rdOpenKeyset, rdConcurReadOnly) ' ) ', rdConcurLock)
DoEvents
...
If numarchi = 3 Then ' para codigos alternos
 ...
 LV1.ColumnHeaders.Add 2, , "Descripci�n", 4000
...
ElseIf numarchi = 1 Then
 LV1.ColumnHeaders.Add 1, , "Descripci�n", 3800
 LV1.Width = 11500
 LV1.Left = 300
 LV1.ColumnHeaders.Add 2, , "Cod.", 600
 LV1.ColumnHeaders.Add 3, , "Direcci�n", 3800
 LV1.ColumnHeaders.Add 4, , "Zona", 1500
ElseIf numarchi = 0 Then
 LV1.ColumnHeaders.Add 1, , "Descripci�n", 4200
...
ElseIf numarchi = 7 Then
 LV1.ColumnHeaders.Add 1, , "Descripci�n", 4450
 LV1.ColumnHeaders.Add 2, , "Cod.", 0
 ...
 LV1.ColumnHeaders.Add 5, , "Pre.Und", 1155, lvwColumnRight
 LV1.ColumnHeaders.Add 6, , "Pre.Pqt", 1155, lvwColumnRight
 LV1.ColumnHeaders.Add 7, , "Precio2", 0
 LV1.ColumnHeaders.Add 8, , "Precio3", 0
 LV1.ColumnHeaders.Add 9, , "Precio4", 0
 LV1.ColumnHeaders.Add 10, , "Equiv", 0
 LV1.ColumnHeaders.Add 11, , "Equiv", 0
 LV1.ColumnHeaders.Add 12, , "Equiv", 0
                          
                                                     
 LV1.ColumnHeaders.Add 13, , StockC2, 0
      
                                                               
        
 LV1.ColumnHeaders.Add 14, , "Stock 2 ", 0
 LV1.ColumnHeaders.Add 15, , StockC3, 0
 ...
 LV1.ColumnHeaders.Add 16, , "Stock 3 ", 0
 
 LV1.ColumnHeaders.Add 17, , " ", 0
 LV1.ColumnHeaders.Add 18, , " ", 0
 LV1.ColumnHeaders.Add 19, , " ", 0
 LV1.ColumnHeaders.Add 20, , " ", 0
 LV1.ColumnHeaders.Add 21, , StockC4, 0
 ...
 LV1.ColumnHeaders.Add 22, , "", 0 ' j. Olmedo UNIDADES
 LV1.ColumnHeaders.Add 23, , StockC5, 0 ' j. Olmedo
 LV1.ColumnHeaders.Add 24, , "", 0 ' j. Olmedo UNIDADES
 LV1.ColumnHeaders.Add 25, , StockC6, 0 '
 LV1.ColumnHeaders.Add 26, , "", 0 '
 LV1.ColumnHeaders.Add 27, , StockC7, 0 '
 LV1.ColumnHeaders.Add 28, , "", 0 '
 LV1.ColumnHeaders.Add 29, , "", 0 '
 LV1.ColumnHeaders.Add 30, , "", 0 '
 LV1.ColumnHeaders.Add 31, , "Familia", 2500
 LV1.Top = 1300
 LV1.Left = 100 ' 1500
...
Do Until X.EOF  ' Or X.AbsolutePosition - 1 >= wmaximo
   If numarchi = 7 Then
     Set itmX = LV1.ListItems.Add(, , Trim(X!art_nombre))
     itmX.SubItems(1) = Trim(X!art_key)
     itmX.SubItems(2) = Trim(X!PRE_UNIDAD)
     itmX.SubItems(3) = Format(X!arm_stock / X!PRE_EQUIV, "0.00")
     itmX.SubItems(4) = "S/. " & Format(X!PRE_PRE1, "0.00")
     itmX.SubItems(5) = "S/. " & Format(X!PRE_PRE2, "0.00")
     itmX.SubItems(6) = Format(X!PRE_PRE3, "0.00")
     itmX.SubItems(7) = Format(X!PRE_PRE4, "0.00")
     If LK_CODUSU = "SUPERVISOR" Or LK_CODUSU = "ADMIN" Then
       itmX.SubItems(8) = Format(X!PRE_PRE5, "0.00")
       itmX.SubItems(11) = Format(X!PRE_PRE6, "0.00")
     End If
     itmX.SubItems(9) = X!PRE_EQUIV
     itmX.SubItems(10) = X!arm_stock
     itmX.SubItems(12) = Format(X!arm_stock2 / X!PRE_EQUIV, "0.00")
     itmX.SubItems(13) = X!arm_stock2
     itmX.SubItems(14) = Format(X!ARM_STOCK3 / X!PRE_EQUIV, "0.00")
     itmX.SubItems(15) = X!ARM_STOCK3
     itmX.SubItems(16) = X!art_marca
     itmX.SubItems(18) = X!art_numero
     itmX.SubItems(19) = X!art_estado
     itmX.SubItems(20) = Format(X!arm_stock4 / X!PRE_EQUIV, "0.00")
     itmX.SubItems(21) = X!arm_stock4
     itmX.SubItems(22) = Format(X!arm_stock5 / X!PRE_EQUIV, "0.00")
     itmX.SubItems(23) = X!arm_stock5
     ...
     itmX.SubItems(28) = Format(Nulo_Valor0(X!PRE_OP1), "0.00")
     itmX.SubItems(29) = Nulo_Valor0(X!PRE_CANT)
    itmX.SubItems(30) = X!TAB_NOMLARGO
     If X!art_marca = 1 Then
        itmX.SubItems(17) = "Lote: " & Jala_lotes(X!art_key, LK_CODCIA, X!PRE_EQUIV)
     Else
        itmX.SubItems(17) = ""
     End If
     
    
     'CAMBIAMOS COLOR RENTABLES - JLPV
     If X!art_familia = 113 Or X!art_familia = 444 Or X!art_familia = 521 Or X!art_familia = 456 Then
        itmX.ForeColor = QBColor(9)
     End If
     'CAMBIAMOS COLOR CONTROLADOS - JLPV
    If X!art_subgru = 3 Or X!art_subgru = 4 Or X!art_subgru = 15 Or X!art_subgru = 106 Or X!art_subgru = 222 Or X!art_subgru = 223 Or X!art_subgru = 224 Or X!art_subgru = 225 Or X!art_subgru = 226 Or X!art_subgru = 227 Or X!art_subgru = 228 Or X!art_subgru = 229 Or X!art_subgru = 230 Or X!art_subgru = 231 Then
        itmX.ForeColor = QBColor(2)
    End If
    'CAMBIAMOS COLOR SIN STOCK - JLPV
     If X!arm_stock = 0 Then
        itmX.ForeColor = QBColor(8)
     End If
   ...
   If numarchi = 0 Or numarchi = 3 Then
    '' itmX.SubItems(2) = Trim(CStr(X.rdoColumns(0)))
      ...
      Else
        itmX.SubItems(2) = Format(X.rdoColumns(4) / X.rdoColumns(5), "0.000")
        itmX.SubItems(3) = Trim(X.rdoColumns(3))
      End If
   End If
   If numarchi = 0 Then
    If X!art_situacion = 1 Then itmX.SubItems(4) = "A"
   End If
...
'DoEvents
    Exit Sub
PROC_LISVIEW_ERR:
    LogError "PROC_LISVIEW: Error " & Err.Number & " - " & Err.Description
    Resume Next
End Sub
...
If par_llave.EOF Then
  MsgBox "NO Existe Compa�ia Avisar !!!! Posible Error .. Intente Nuevamente ....", 48, Pub_Titulo
  End
  Exit Sub
End If
...
If Trim(LK_MONEDA) = "" Then MsgBox "DEFINIR LA MONEDA DE COMPA�IA", 48, Pub_Titulo
...
LK_RUTA_DOC = Trim(Nulo_Valors(par_llave!PAR_RUTA_REPO)) '"P:\ADMIN\NORDI\DISTRI\" PE�AMEZA
LK_FLAG_SERIES = "A"
If IsNull(par_llave!PAR_FECHA_DIA) Then
  MsgBox "URGENTE!!!. Esta Compa�ia No Tiene Definida la Fecha de Trabajo. Verificar!!!", 48, Pub_Titulo
...
LK_DEVICE_FBG = Nulo_Valors(par_llave!PAR_DEVICE_FBG)
LK_FLAG_PARTES = Nulo_Valors(par_llave!PAR_FLAG_PARTE)
...
lk_descrip_cias = Trim(par_llave!par_nombre_corto)
...
lk_descrip_cias = Trim(lk_descrip_cias) + Trim(par_multi!par_nombre_corto)
If LK_CODCIA = "01" Or LK_CODCIA = "07" Or LK_CODCIA = "03" Or LK_CODCIA = "10" Or LK_CODCIA = "26" Or LK_CODCIA = "25" Then
 flag_cambio_pre = "A"
Else
 flag_cambio_pre = ""
End If
...
Exit Sub
ecolor:
' MsgBox "Verificar los codigos de color en OSBusiness", 48, Pub_Titulo
End Sub
...
Dim flag_fd As String * 1
Dim flag_cc As String * 1
Dim SUM_IMPORTE As Double
Dim SUM_CC As Double
    ...
    LEER_CAR_LLAVE
    Do Until car_mayor.EOF
      If car_mayor!CAR_CP = "C" Then
       If (car_mayor!CAR_TIPDOC = "FA" Or car_mayor!CAR_TIPDOC = "CC" Or car_mayor!CAR_TIPDOC = "PT" Or car_mayor!CAR_TIPDOC = "PA" Or car_mayor!CAR_TIPDOC = "RD") Then
       Else
         GoTo pasa
       End If
      End If
       ...
       If car_mayor!car_importe > 0 Then
          ...
          If car_mayor!CAR_FECHA_INGR <> LK_FECHA_DIA Then flag_fd = "A"
          If (car_mayor!CAR_TIPDOC = "CC") And car_mayor!CAR_FECHA_INGR <> LK_FECHA_DIA Then PUB_CC_FUFE = "A"
       ...
       If car_mayor!CAR_TIPDOC <> "CH" Then
        ...
        Else
           ...
           If car_mayor!CAR_FECHA_INGR <> LK_FECHA_DIA Then flag_cc = "A"
        End If
        End If
...
pasa:
    ...
    If flag_cc = "A" Then PUB_CC_VENTA = 9 ' CERO PROCEDE LA VENTA  9 BLOQUEA
    If Trim(flag_fd) = "A" Then PUB_FLAG_VENCIDO_VISTA = 9
    CAR_TOT_CPX2 = SUM_IMPORTE
    pub_mensaje = pub_mensaje & "TOTAL CREDITO =  " & SUM_IMPORTE & Chr(13) & "TOTAL CONTADO REPARTO =  " & SUM_CC
    
End Function
Public Function CAR_TOT_CC(wcp As String, wcodcia As String, wcodclie As Double, Optional adddia) As Double
Dim wdiamas As Integer
If Not IsMissing(adddia) Then wdiamas = adddia Else wdiamas = 0
Dim SUM_IMPORTE As Double
    ...
    Do Until car_mayor.EOF
       If car_mayor!CAR_TIPDOC <> "CC" Then GoTo pasa
         ...
         End If
       If car_mayor!car_importe > 0 Then
          If DateDiff("d", car_mayor!car_fecha_vcto, LK_FECHA_DIA) > (Val(Nulo_Valor0(par_llave!PAR_DIAS_VENC_CC)) + Val(wdiamas)) Then PUB_FLAG_VENCIDO_CC = 1
       End If
       pub_mensaje = pub_mensaje & car_mayor!CAR_TIPDOC & "   - " & Nulo_Valors(car_mayor!car_fbg) & "/. " & car_mayor!car_NUMSER & " - " & car_mayor!car_NUMFAC & "       F.Vcto: " & car_mayor!car_fecha_vcto & "        " & car_mayor!CAR_MONEDA & "/." & Format(car_mayor!car_importe, "0.00") & "     -V: " & car_mayor!CAR_codven & Chr(13) & Chr(13)
            ...
            Case 4
              ...
              If WNUM = 14 Then
                 If LK_ACCESO_REPORT <> "A" Then
                   MDIForm1.SubmenuTit4.Item(WNUM).Enabled = False
                 End If
               End If
            Case 5
...
If LK_CODUSU = "ADMIN" Or LK_CODUSU = "SUPER" Then MDIForm1.Toolbar1.Buttons(14).Enabled = True
...
For fila = 0 To MDIForm1.menudis3.count - 1
 ...
 End If
Next fila
For fila = 0 To MDIForm1.menudis9.count - 1
 If fila <> 0 Then
   Unload MDIForm1.menudis9(fila)
 Else
   MDIForm1.menudis9(fila).Caption = ""
 End If
Next fila
For fila = 0 To MDIForm1.menudis10.count - 1
 If fila <> 0 Then
   Unload MDIForm1.menudis10(fila)
 Else
   MDIForm1.menudis10(fila).Caption = ""
 End If
Next fila
For fila = 0 To MDIForm1.menudis11.count - 1
 If fila <> 0 Then
   Unload MDIForm1.menudis11(fila)
 Else
   MDIForm1.menudis11(fila).Caption = ""
 End If
Next fila
For fila = 0 To MDIForm1.menudis12.count - 1
 If fila <> 0 Then
   Unload MDIForm1.menudis12(fila)
 Else
   MDIForm1.menudis12(fila).Caption = ""
 End If
Next fila
...
'  Exit Function
'End If
wRuta = LK_RUTA_DOC 'PUB_RUTA_OTRO
SQ_OPER = 2
PUB_CODCIA = LK_CODCIA
PUB_CODVEN = Val(FORMGEN.i_codven.Text)
LEER_PAR_LLAVE
If pac_llave.EOF Then
'   MsgBox "No se ha definido archivos de Impresi�n", 48, Pub_Titulo
'   Exit Function
End If
...
otras_bol:
        wRuta = Trim(LK_RUTA_DOC) & "V10\"
        WNUM = Mid(WDEVI, 3, 1)
      End If
      On Error GoTo SALIMP
      If pub_puerto = "1" Then
        ctaprint = 0
    ...
    If Trim(FORMGEN.i_fbg.Text) = "B" Or Trim(FORMGEN.i_fbg.Text) = "P" Then
        FORMGEN.Reportes.WindowTitle = "BOLETA  :" & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "00000000")
        If ws_sut_transa = 20 Or ws_sut_transa = 24 Then
            FORMGEN.Reportes.ReportFileName = wRuta & "ELCLIBOL.RPT"
        Else
            FORMGEN.Reportes.ReportFileName = wRuta & "CLIBOL.RPT"
        End If
    ElseIf Trim(FORMGEN.i_fbg.Text) = "F" Then
        FORMGEN.Reportes.WindowTitle = "FACTURA : " & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "00000000")
        If ws_sut_transa = 20 Or ws_sut_transa = 24 Then
            FORMGEN.Reportes.ReportFileName = wRuta & "ELCLIFAC.RPT"
        Else
            FORMGEN.Reportes.ReportFileName = wRuta & "CLIFAC.RPT"
        End If
    ElseIf Trim(FORMGEN.i_fbg.Text) = "G" Then
      FORMGEN.Reportes.WindowTitle = " GUIA   : " & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "00000000")
      FORMGEN.Reportes.ReportFileName = wRuta & "CLIGUI.RPT"
    End If
...
'MsgBox Err.Number & Err.Description
 ...
 MsgBox "Se produjo un conflicto Nro:" & Err.Number & Chr(13) & "El Sistema Intentar� mostrar la impresi�n por pantalla", vbInformation, Pub_Titulo
...
If Trim(Nulo_Valors(llave_rep01!REP_CIA)) <> "" Then
  VALOR = Mid(MDIForm1.TXTCIA.Caption, 4, Len(MDIForm1.TXTCIA.Caption))
  For I = 1 To loc_xl.Names.count
...
If numarchi = 99 Then ' para codigos alternos
 LV1.ColumnHeaders.Add 1, , "Descripci�n", 4000
...
'DoEvents
    Exit Sub
PROC_TRANSA_ERR:
    LogError "PROC_TRANSA: Error " & Err.Number & " - " & Err.Description
    Resume Next
End Sub
...
ElseIf par_MOV_TIPMOV = 3 Then
 par_MOV_DETALLE = "Res�men Egresos caja "
...
Do Until FLAGX = "A"
   ...
   If Val(EE.Cells(correla, 1)) = 0 Then
     If UCase(Left(Trim(EE.Cells(correla, 1)), 5)) = "TOTAL" And Val(EE.Cells(correla + 1, 1)) <> 0 Then
        par_MOV_NRO_MOV = 0
        par_MOV_NRO_VOUCHER = par_MOV_NRO_VOUCHER + 1
        par_MOV_DETALLE = "Res�men Ingresos de Caja "
...
Do Until ch_llave.EOF
      ...
      If fa_llave.EOF Then
        MsgBox "Cheque no Aplicado a ningun Documento " & Chr(13) & " OJO  - A N O T A R :" & Chr(13) & "N�.Ch/: " & ch_llave!car_num_cheque & Chr(13) & "N� Documento : " & Trim(ch_llave!car_concepto) & Chr(13) & "Importe : " & Format(ch_llave!car_importe, "##,##0.00"), 48, Pub_Titulo
        GoTo bien
      End If
...
Public Function DeudaDia(wcp As String, wfechavcto As Date, Optional IDFlag) As Currency
Dim ps_lelr As rdoResultset
Dim wsp_codcia As String
If Not IsMissing(IDFlag) Then
    If LK_CODCIA = "01" Or LK_CODCIA = "03" Or LK_CODCIA = "07" Or LK_CODCIA = "10" Then
        wsp_codcia = "('01','03','07','10')"
    Else
        wsp_codcia = "(" & LK_CODCIA & ")"
    End If
    DeudaDia = 0
...
Else
    If LK_CODCIA = "25" Or LK_CODCIA = "27" Or LK_CODCIA = "28" Or LK_CODCIA = "29" Then
        wsp_codcia = "('01','07','05','03','25','27','28','29')"
    Else
        wsp_codcia = "(" & LK_CODCIA & ")"
    End If
    DeudaDia = 0
    ...
    'PSCAR_TOPE(1) = wcp
    'PSCAR_TOPE(2) = wfechavcto
    'car_tope.Requery
    'DeudaDia = 0
    'Do Until car_tope.EOF
    'DeudaDia = DeudaDia + Nulo_Valor0(car_tope!total)
    'car_tope.MoveNext
    'Loop
...
Public Function Jala_lotes(wt_key As Currency, wt_codcia As String, wt_equiv As Integer) As String
Dim rdlot As rdoResultset

pub_cadena = "SELECT * FROM LOTE WHERE LOT_CODCIA = '" & wt_codcia & "' AND LOT_CODART = " & wt_key & " AND LOT_SALDOS <> 0  ORDER BY LOT_FECHA_VCTO "
Set rdlot = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
rdlot.Requery
Jala_lotes = ""
'If rdlot.RowCount > 1 Then Stop
Do Until rdlot.EOF
  Jala_lotes = Jala_lotes + " " + Trim(rdlot!LOT_NROLOTE) & " - " & Format(rdlot!lot_fecha_vcto, "dd/mm/yyyy") & " Stock: " & Format(rdlot!LOT_SALDOS / wt_equiv, "0.00")
  rdlot.MoveNext
Loop
...
Else
  If ws_equiv <> 0 Then
    StockGenAlm = Format(ps_stockg!STOCKG / ws_equiv, "0.00")
  Else
    StockGenAlm = ps_stockg!STOCKG
  End If
End If
End Function

Public Function Ver_bloq_libros(WTIPMOV As Integer) As Boolean
pub_cadena = "SELECT * FROM BLOLIBROS where blo_codcia = '" & cop_llave!cop_CODCIA & "' and blo_periodo = " & Val(Format(cop_llave!cop_fecha_proceso, "yyyy")) & " and blo_mes = " & Val(Format(cop_llave!cop_fecha_proceso, "mm")) & "  and blo_tipmov = " & WTIPMOV & " and blo_estado = '1'"
Set X = CN.OpenResultset(pub_cadena, rdOpenKeyset, rdConcurValues) ' rdConcurReadOnly) ', rdConcurLock)
If X.EOF Then
Ver_bloq_libros = False
Else
Ver_bloq_libros = True
End If
