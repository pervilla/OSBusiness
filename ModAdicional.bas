Attribute VB_Name = "ModAdicional"
...
If wpo_estado = "E" Then
  If po_aviso = 0 Then
     MsgBox "La Condicion de Gasto, No definida en el Presupuesto, No procede la operaciÛn ", 48, Pub_Titulo
     Presup_Oper = False
     Exit Function
  End If
End If
...
If po_aviso = 0 Then
 If wpo_presup = 0 Then
    MsgBox "Este Tipo de Gasto No tiene Presupuesto, No procede la operaciÛn ", 48, Pub_Titulo
    Presup_Oper = False
    Exit Function
 ElseIf wpo_impacu > wpo_presup Then
     MsgBox "Monto Supera a lo presupuestado, No procede la operaciÛn ", 48, Pub_Titulo
     Presup_Oper = False
      Exit Function
 End If
