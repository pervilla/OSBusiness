...
If par_llave!PAR_FECHA_DIA <> LK_FECHA_DIA Then
   MsgBox "!La Fecha del Sistema ha cambiado, ya se cerro el día de la compañia ..!, el sietma se cerrarro  ", 48, Pub_Titulo
   End
End If
...
PASA_POR_CIAS:
   ...
   If Val(tab_llave!tab_codclie) <> Val(PUB_NUM_OPER_XXX - 1) Then
       MsgBox "Falta Procesar el Reporte de Caja en SOLES (S/.). " & Chr(13) & "Codigo de Caja Nro  : " & Format(PUB_NUMTAB, "00"), 48, Pub_Titulo
       GoTo SALIDA_ERROR
  End If
  WS_CAJSALDO_C1 = Val(tab_llave!TAB_contable2)
   ...
   If (Val(WS_CAJSALDO_C1) + Val(WS_CAJSALDO_C2) + Val(WS_CAJSALDO_C3)) <> WS_SALDO_S Then
      MsgBox "Falta Revisar o Procesar las Cajas no cuadran las Internas en SOLES (S/.). " & Chr(13) & "Codigo de Caja Nro  : " & Format(PUB_NUMTAB, "00"), 48, Pub_Titulo
      GoTo SALIDA_ERROR
   End If
...
solo_cierre:
   ...
   cal_llave.Update
   'agfkn ro asghdíahgro khagfdsro kafdsga sgfda sdfgasfd
   GoSub manda_numero
   If Trim(par_llave!par_ruta_imp) <> "" Then
...
'   Label3.Caption = "ALMACEN Y PUNTOS DE VENTAS"
   ...
   End If
Else
'   Label3.Caption = "C O M P A ro  I A"
   SQ_OPER = 1
   PUB_CODCIA = LK_CODCIA
   LEER_PAR_LLAVE
...
If Not FAR_TRANS.EOF Then
    ...
    If Trim(LK_CODUSU) = "ADMIN" Or Trim(LK_CODUSU) = "SUPER" Then
       MsgBox "Existe Mercaderia en Transito, Verificar con las Operaciónes de Recepciro n de Mercaderia, Tiene el acceso!!!!"
    Else
       MsgBox "Existe Mercaderia en Transito, Verificar con las Operaciónes de Recepciro n de Mercaderia, No Procede,"
       Unload PRODIA
       Exit Sub
    End If
...
If Not FAR_TRANS.EOF Then
    ...
    '   MsgBox "Existe Mercaderia en Transito, Verificar con las Operaciónes de Recepciro n de Mercaderia, Tiene el acceso!!!!"
    'Else
       MsgBox "Existe Efectivo Transito, Verificar con las Operaciónes de Recepciro n de Efectivo Operación: 5375, No Procede,"
       Unload PRODIA
       Exit Sub
    'End If
End If
