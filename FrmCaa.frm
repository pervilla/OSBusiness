...
Begin VB.Form FrmCaa 
   ...
   Begin VB.Label momento 
      Alignment       =   2  'Center
      Caption         =   "Procesando Informaci�n. un momento..."
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
Dim Mensaje, T�tulo, valorpred, mifecha
If Val(i_codcli.Text) = 0 Then i_codcli.SetFocus: Exit Sub
If Index = 1 Then
T�tulo = "Movimientos de activaci�n de Bloqueo 2007"
Else
T�tulo = "Movimientos de Limite de Credito de Clientes"
End If
Mensaje = "Ingrese una Fecha de Inicio para la Consulta : "
valorpred = Format(LK_FECHA_DIA, "dd/mm/yyyy")
mifecha = InputBox(Mensaje, T�tulo, valorpred)
If mifecha = "" Then
   Exit Sub
End If
...
If GRIDG.COL <> 15 Then Exit Sub
 wcodusu = InputBox("Fecha de Emisi�n", "Cambio Fecha", GRIDG.TextMatrix(GRIDG.Row, 15))
...
If LK_CODUSU = "ADMIN" And (GridK.COL = 13 Or GridK.COL = 1) Then
   If GridK.COL = 13 Then
    ...
    End If
   Else
     wcodusu = InputBox("Fecha de Emisi�n", "Cambio Fecha", GridK.TextMatrix(GridK.Row, 1))
    If wcodusu = "" Then Exit Sub
    If Not IsDate(wcodusu) Then Exit Sub
   End If
 pu_numdoc = Val(GridK.TextMatrix(GridK.Row, 8))
  ...
  End If
    If wcar_mayor!CAR_CP = "C" Then
     ...
     ElseIf Trim(wcar_mayor!car_fbg) = "" Then
        vdocum = "N�:" & wcar_mayor!car_NUMSER & "-" & wcar_mayor!car_NUMFAC
     Else
        vdocum = " "
     End If
     End If
    ...
    If wcar_mayor!CAR_TIPMOV = 0 Or wcar_mayor!CAR_TIPDOC = "CD" Or wcar_mayor!CAR_TIPDOC = "CH" Then
      ...
      ' Else
      '    vdocum = "N�. " & wcar_mayor!car_NUM_CHEQUE
      ' End If
